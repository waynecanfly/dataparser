...
from algorithm.character_converge_pdf.container.PageSeg import PageSeg
from algorithm.character_converge_pdf.container.line import Line
from algorithm.character_converge_pdf.container.linetable import LineTable
from algorithm.character_converge_pdf.gen_line_table import gen_line_table_tools
...
def split_by_V(v_seg_map):
    ...
    for i in range(1, len(seg_point)):
        ...
        # 计算check_seg和所有垂直线段的重叠数，如果重叠数少于3， 则该check_seg所表示的范围不应该由表，则断开
        ov_num = overlapNum(check_seg, v_seg_map.values())
        if ov_num < 3 and legal_start_point is not None:
            table_v_seg_box.append([legal_start_point, startpoint])
            legal_start_point = None
        elif ov_num >=3 and legal_start_point is None:
            legal_start_point = startpoint
        # 如果时最后一个点，且上一段时有效的，则应生成新的垂直有效线段
        if i+1 == len(seg_point) and ov_num >=3 :
