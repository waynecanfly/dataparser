...
def getRecognizeLib():
    currency_sql = 'select content,currency,country_code from currency_measureunit_mapping_lib where mapping_type = 1'
    measureunit_sql = 'select content,measureunit,country_code from currency_measureunit_mapping_lib where mapping_type = 2'
    currency_measureunit_sql = 'select content,currency,measureunit,country_code from currency_measureunit_mapping_lib where mapping_type = 3'
...
def recognize_currency_measureunit_iscon(text, lib, country_code, is_header):
    ...
    pureText = re.sub('\s|#_#', '', text.lower())
    ...
    if result['measureunit'] is None:
        ...
        for key in lib.measureunit_lib:
            pute_key = re.sub('\s', '', key.lower())
            if pute_key in pureText:
