...
class Table:
    ...
    def findTitle(self, header_beginindex):
        ...
        # 如果本页剩余的title少于三行，则往上一页再找三行
        title_linebox = self.lastpage.linebox[-3:] + title_linebox if self.lastpage is not None and len(title_linebox) < 3 else title_linebox
    ...
    def valueColumnLegal(self, line, minAndMax):
        for b in line.blockbox:
            if b.x1 > minAndMax[0] and b.type in  ['text-sentence', 'text-phrase']:
                return False
        return True
